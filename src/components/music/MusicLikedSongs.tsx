      ...
      return {
        ...
        queryFn: async (): Promise<UnifiedTrack> => {
          ...
          if (hasMetadata) {
            ...
            const mediaUrlTag = likedSongs?.tags.find(tag => tag[0] === 'track-media-url' && tag[1] === url);
            const trackUrlTag = likedSongs?.tags.find(tag => tag[0] === 'track-url' && tag[1] === url);
            const durationTag = likedSongs?.tags.find(tag => tag[0] === 'track-duration' && tag[1] === url);
